...
object LoadImageAction extends Action[EnvironmentWithExecutionContext,String,Uint8Array,LoadImageErrors]{
  ...
  def execute(filepath: String)
    ...
    * implicit val ec = environment.getExecutionContext()
    for {
      checkFilePath ← checkFileAccess(filepath)
      image ← EitherT.left(IO.fromFuture(IO(typings.node.fsPromisesMod.readFile(filepath).toFuture)))
      uint8 ← EitherT.left(IO(image.asInstanceOf[Uint8Array]))
    } yield uint8
    */
  }
